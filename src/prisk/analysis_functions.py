...
def simple_risk_overlay(
    flood_path, exposure_path, output_path, damage_function, depth_adjuster
):
    """
    ...
    with rasterio.open(output_path, "w", **profile) as dst:
        ...
        for ji, window in flood.block_windows(1):
            ...
            profile.update({"height": height, "width": width, "affine": affine})

            flood_array = flood.read(1, window=window)
            exposure_array = exposure.read(1, window=window)
            flood_array = np.where(
                flood_array > 0, flood_array, 0
            )  # remove negative values
            flood_array = flood_array / depth_adjuster  # convert to m
            risk = calculate_risk(
                flood_array, exposure_array, damage_function[0], damage_function[1]
            )  # depths index 0 and prp damage index 1
...
def create_relative_exposure_layer(exposure_path, total_exposure_value, output_path):
    ...
    with rasterio.open(output_path, "w", **profile) as dst:
        ...
        for ji, window in exposure.block_windows(1):
            ...
            exposure_array = exposure.read(1, window=window)
            exposure_array = np.where(
                exposure_array > 0, exposure_array / total_exposure_value, 0
            )  # calculate proportional area rather than absolute (may have to adjust this for decimal precision)

            dst.write(exposure_array.astype(rasterio.float32), window=window, indexes=1)


def dignad_simple_risk_overlay(
    flood_path,
    exposure_path,
    output_path,
    damage_function,
...
def simple_risk_overlay_percentile(
    flood_path, exposure_path, output_path, damage_function, depth_adjuster
):
    """
    TODO
...
def flopros_risk_overlay(
    flood_path, exposure_path, output_path, mask_path, damage_function, depth_adjuster
):
    """
    ...
    with rasterio.open(output_path, "w", **profile) as dst:
        ...
        for ji, window in flood.block_windows(1):
            ...
            exposure_array = np.where(
                mask_array == 1, 0, exposure_array
            )  # wherever the urban mask equals 1, set to zero in exposure dataset
            flood_array = np.where(
                flood_array > 0, flood_array, 0
            )  # remove negative values
            flood_array = flood_array / depth_adjuster  # convert to m
...
def dryproofing_risk_overlay(
    ...
    with rasterio.open(output_path, "w", **profile) as dst:
        ...
        for ji, window in flood.block_windows(1):
            ...
            exposure_array = exposure.read(1, window=window)
            mask_array = mask.read(1, window=window)
            dry_proof_array = np.where(
                mask_array == 1, exposure_array, 0
            )  # cells for dry proofing are marked as 1
            non_dry_proof_array = np.where(mask_array == 1, 0, exposure_array)
            flood_array = np.where(
                flood_array > 0, flood_array, 0
            )  # remove negative values
            flood_array = flood_array / depth_adjuster  # convert to m
            dry_proof_risk = calculate_risk(
                flood_array,
                dry_proof_array,
                damage_function_dp[0],
                damage_function_dp[1],
            )  # pass new damage function for dry proof cells
            non_dry_proof_risk = calculate_risk(
                flood_array, non_dry_proof_array, damage_function[0], damage_function[1]
...
def relocation_risk_overlay(
    ...
    with rasterio.open(output_path, "w", **profile) as dst:
        ...
        for ji, window in flood.block_windows(1):
            ...
            mask_array = mask.read(1, window=window)
            exposure_array = np.where(
                mask_array > 0, 0, exposure_array
            )  # Remove cells that will be relocated
            flood_array = np.where(
                flood_array > 0, flood_array, 0
            )  # remove negative values
            flood_array = flood_array / depth_adjuster  # convert to m
...
def combine_glofas(start, end, dir, area_filter):
    ...
    """
    all_files = [
        os.path.join(dir, f"glofas_THA_{year}.grib") for year in range(start, end + 1)
    ]  # if we do this for other countries will have to adjust filenames
    ...
    # Load upstream area
    upstream_area = xr.open_dataset(
        os.path.join(dir, "uparea_glofas_v4_0.nc"), engine="netcdf4"
    )  # might need to update the filename here
    ...
    lat_slice_index = [
        round((i - up_lats[0]) / (up_lats[1] - up_lats[0])) for i in lat_limits
    ]
    lon_slice_index = [
        round((i - up_lons[0]) / (up_lons[1] - up_lons[0])) for i in lon_limits
    ]
    ...
    combined_dataset_masked = combined_dataset.where(
        combined_dataset.uparea >= area_filter * 1e6
    )
...
def perform_outlet_data_checks(outlets, discharge_data):
    ...
    for index, row in outlets.iterrows():
        ...
        if not valid:
            print(
                f"ID: {row['HYBAS_ID']}, Lat: {latitude}, Lon: {longitude}, Acc: {acc}, Valid: {valid}, Message: {message}"
            )
...
def fit_gumbel_distribution(basin_timeseries):
    ...
    for basin_id, timeseries in basin_timeseries.items():
        ...
        D, p_value = kstest(
            annual_maxima, "gumbel_r", args=(params["loc"], params["scale"])
        )
...
def calculate_uniform_marginals(basin_timeseries, gumbel_parameters):
    ...
    for basin_id, timeseries in basin_timeseries.items():
        ...
        annual_maxima = timeseries.groupby(timeseries.index.year).max()
        uniform_marginals[basin_id] = gumbel_r.cdf(
            annual_maxima, loc=params["loc"], scale=params["scale"]
        )
...
def calculate_basin_copula_pairs(uniform_marginals, plot_dependence_matrix=False):
            ...
            if id1 < id2:  # to avoid duplicate pairs
                try:
                    ...
                    data = np.column_stack(
                        (1 - margins1, 1 - margins2)
                    )  # interested in upper tail dependence so take inverse of CDF TODO: NEED TO CHECK IF THIS IS VALID
    ...
    basin_ids = list(
        uniform_marginals.keys()
    )  # take the basin IDs from the uniform marginals dictionary
    ...
    if plot_dependence_matrix:
        plt.imshow(
            dependence_matrix, cmap="viridis", interpolation="none", vmin=0, vmax=3
        )
        plt.colorbar()
...
def minimax_ordering(dependence_matrix, basin_ids):
    ...
    masked_dependence_matrix = np.ma.masked_less(
        dependence_matrix, 0
    )  # masking out values < 0
    np.fill_diagonal(
        masked_dependence_matrix, np.ma.masked
    )  # we want to ignore diagonal (NaN values)
    max_theta_index = np.unravel_index(
        np.argmax(masked_dependence_matrix, axis=None), masked_dependence_matrix.shape
    )
    ...
    while len(ordered_basins) < len(basin_ids):
        ...
        potential_next_indices = [
            i for i in range(len(basin_ids)) if i not in selected_indices
        ]
        ...
        for idx in selected_indices:
            min_deps_masked.mask[idx] = (
                True  # mask the index if it's already in selected indices
            )
...
def generate_conditional_sample(v, theta, r):
    ...
    if theta > 30:
        theta = (
            30  # need to determine what this threshold should be via trial and error.
        )

    u = 1 - (1 + ((1 - v) ** (-theta)) * (((r ** (-((theta) / (1 + theta)))) - 1))) ** (
        -(1 / theta)
    )
...
def d_interpolate_damages(
    RPs, losses, sim_aep, total_cs, proportional_cs, protection_level=0.5
):
    """
    Function to interpolate damages between given an annual exceedance probability and
...
def basin_loss_curve(
    loss_df, basin_id, basin_col, epoch_val, scenario_val, urban_class, rps
):
    """
    ...
    basin_df = loss_df[
        (loss_df[basin_col] == basin_id)
        & (loss_df["epoch"] == epoch_val)
        & (loss_df["adaptation_scenario"] == scenario_val)
        & (loss_df["urban_class"] == urban_class)
    ]
    grouped_basin_df = (
        basin_df.groupby([basin_col, "RP", "Pr_L_AEP"])
        .agg({"damages": "sum"})
        .reset_index()
    )
    ...
    for i in unique_protection_levels:
        losses[i] = [
            grouped_basin_df.loc[
                (grouped_basin_df["RP"] == rp) & (grouped_basin_df["Pr_L_AEP"] == i),
                "damages",
            ].sum()
            for rp in rps
        ]
...
def monte_carlo_dependence_simulation(
    ...
    random_numbers = np.random.uniform(
        0, 1, (num_simulations, num_years, len(ordered_basins))
    ).astype(np.float32)
                ...
                if i == 0:
                    ...
                    for Pr_L in loss_curves:  # loop through basin protection levels
                        if Pr_L <= r:
                            ...
                            yearly_loss_values.append(
                                interpolate_damages(
                                    rps, loss_curves[Pr_L], r, protection_level
                                )
                            )
                    ...
                    copula = get_copula_model(
                        copula_models, ordered_basins[i - 1], basin_id
                    )
                    if copula is not None:
                        # Apply dependency model if theta exists
                        r = random_numbers[simulation, year, i]
                        previous_loss = yearly_losses[i - 1]
                        current_loss = generate_conditional_sample(
                            previous_loss, copula.theta, r
                        )
    ...
    return pd.DataFrame(
        national_losses_per_year, columns=[f"Year_{i+1}" for i in range(num_years)]
    )


def urban_monte_carlo_dependence_simulation(
    loss_df,
    rps,
    basin_col,
    epoch_val,
    ...
    basin_loss_curves = {
        basin_id: basin_loss_curve(
            loss_df, basin_id, basin_col, epoch_val, "Baseline", urban_class, rps
        )
        for basin_id in ordered_basins
    }
    # Initialize array for national losses
...
def sectoral_monte_carlo_dependence_simulation(
    loss_df,
    rps,
    basin_col,
    epoch_val,
    ...
    res_basin_loss_curves = {
        basin_id: basin_loss_curve(
            loss_df, basin_id, basin_col, epoch_val, scenario_val, "Residential", rps
        )
        for basin_id in ordered_basins
    }
    com_basin_loss_curves = {
        basin_id: basin_loss_curve(
            loss_df, basin_id, basin_col, epoch_val, scenario_val, "Commercial", rps
        )
        for basin_id in ordered_basins
    }
    ind_basin_loss_curves = {
        basin_id: basin_loss_curve(
            loss_df, basin_id, basin_col, epoch_val, scenario_val, "Industrial", rps
        )
        for basin_id in ordered_basins
    }
    inf_basin_loss_curves = {
        basin_id: basin_loss_curve(
            loss_df, basin_id, basin_col, epoch_val, scenario_val, "Infrastructure", rps
        )
        for basin_id in ordered_basins
    }
                ...
                if i == 0:
                    ...
                    for Pr_L in res_loss_curves:  # loop through basin protection levels
                        if Pr_L <= r:
                            ...
                            res_yearly_loss_values.append(
                                interpolate_damages(
                                    rps, res_loss_curves[Pr_L], r, protection_level
                                )
                            )
                            com_yearly_loss_values.append(
                                interpolate_damages(
                                    rps, com_loss_curves[Pr_L], r, protection_level
                                )
                            )
                            ind_yearly_loss_values.append(
                                interpolate_damages(
                                    rps, ind_loss_curves[Pr_L], r, protection_level
                                )
                            )
                            inf_yearly_loss_values.append(
                                interpolate_damages(
                                    rps, inf_loss_curves[Pr_L], r, protection_level
                                )
                            )
                    ...
                    if copula is not None:
                        ...
                        for (
                            Pr_L
                        ) in res_loss_curves:  # loop through basin protection levels
                            if Pr_L <= current_loss:
                                # print(Pr_L, 'smaller than', r, 'continuing...') # if baseline protection is achieved...
                                continue
                            else:
    ...
    res_df = pd.DataFrame(
        res_national_losses_per_year, columns=[f"Year_{i+1}" for i in range(num_years)]
    )
    com_df = pd.DataFrame(
        com_national_losses_per_year, columns=[f"Year_{i+1}" for i in range(num_years)]
    )
    ind_df = pd.DataFrame(
        ind_national_losses_per_year, columns=[f"Year_{i+1}" for i in range(num_years)]
    )
    inf_df = pd.DataFrame(
        inf_national_losses_per_year, columns=[f"Year_{i+1}" for i in range(num_years)]
    )

    return res_df, com_df, ind_df, inf_df


def sectoral_urban_monte_carlo_dependence_simulation(
    loss_df,
    rps,
    basin_col,
    epoch_val,
    ...
    urban_res_basin_loss_curves = {
        basin_id: basin_loss_curve(
            loss_df, basin_id, basin_col, epoch_val, scenario_val, "Residential", rps
        )
        for basin_id in ordered_basins
    }
    urban_com_basin_loss_curves = {
        basin_id: basin_loss_curve(
            loss_df, basin_id, basin_col, epoch_val, scenario_val, "Commercial", rps
        )
        for basin_id in ordered_basins
    }
    urban_ind_basin_loss_curves = {
        basin_id: basin_loss_curve(
            loss_df, basin_id, basin_col, epoch_val, scenario_val, "Industrial", rps
        )
        for basin_id in ordered_basins
    }
    urban_inf_basin_loss_curves = {
        basin_id: basin_loss_curve(
            loss_df, basin_id, basin_col, epoch_val, scenario_val, "Infrastructure", rps
        )
        for basin_id in ordered_basins
    ...
    res_basin_loss_curves = {
        basin_id: basin_loss_curve(
            loss_df, basin_id, basin_col, epoch_val, "Baseline", "Residential", rps
        )
        for basin_id in ordered_basins
    }
    com_basin_loss_curves = {
        basin_id: basin_loss_curve(
            loss_df, basin_id, basin_col, epoch_val, "Baseline", "Commercial", rps
        )
        for basin_id in ordered_basins
    }
    ind_basin_loss_curves = {
        basin_id: basin_loss_curve(
            loss_df, basin_id, basin_col, epoch_val, "Baseline", "Industrial", rps
        )
        for basin_id in ordered_basins
    }
    inf_basin_loss_curves = {
        basin_id: basin_loss_curve(
            loss_df, basin_id, basin_col, epoch_val, "Baseline", "Infrastructure", rps
        )
        for basin_id in ordered_basins
    }
...
def update_natural_hazard_parameters(
    nat_disaster_year,
    recovery_period,
    tradable_impact,
    nontradable_impact,
...
def run_DIGNAD(
    ...
    for index, row in calibration_df.iterrows():
        parameter = row[
            "Parameters"
        ]  # The column name in your CSV for the parameter names
    ...
    natdisaster_params = update_natural_hazard_parameters(
        nat_disaster_year,
        recovery_period,
        tradable_impact,
        nontradable_impact,
    ...
    file_path = (
        r"D:\projects\sovereign-risk\Thailand\DIGNAD\DIGNAD_Toolkit_2023\PW_SHARED_2023\DIGNAD_Toolkit\DIGNAD_python\Excel output\%s\Model_output_%s.xlsx"
        % (today, today)
    )
...
def dignad_loss_probability(
    ...
    tradable_perc_impact = (
        (res_losses * res_share_tra)
        + (com_losses * com_share_tra)
        + (ind_losses * ind_share_tra)
        + (inf_losses * inf_share_tra)
    ) / tradable_cap_stock
    nontradable_perc_impact = (
        (res_losses * (1 - res_share_tra))
        + (com_losses * (1 - com_share_tra))
        + (ind_losses * (1 - ind_share_tra))
        + (inf_losses * (1 - inf_share_tra))
    ) / nontradable_cap_stock
    private_perc_impact = (
        (res_losses * res_share_pri)
        + (com_losses * com_share_pri)
        + (ind_losses * ind_share_pri)
        + (inf_losses * inf_share_pri)
    ) / GDP
    public_perc_impact = (
        (res_losses * (1 - res_share_pri))
        + (com_losses * (1 - com_share_pri))
        + (ind_losses * (1 - ind_share_pri))
        + (inf_losses * (1 - inf_share_pri))
    ) / GDP
    share_tradable = (
        (res_losses * res_share_tra)
        + (com_losses * com_share_tra)
...
def get_dignad_inputs_for_rp(
    ...
    """
    tradable_impact = get_loss_for_rp(
        return_period, sorted_tradable, exceedance_probabilities
    )
    nontradable_impact = get_loss_for_rp(
        return_period, sorted_nontradable, exceedance_probabilities
    )
    public_impact = get_loss_for_rp(
        return_period, sorted_public, exceedance_probabilities
    )
    private_impact = get_loss_for_rp(
        return_period, sorted_private, exceedance_probabilities
    )
    share_tradable = get_loss_for_rp(
        return_period, sorted_share_tradable, exceedance_probabilities
    )
    return (
        tradable_impact,
        nontradable_impact,
...
def model_dependence(plot=False):
    ...
    clayton_copula_models, clayton_error_basins, dependence_matrix = (
        calculate_basin_copula_pairs(uniform_marginals, plot_dependence_matrix=plot)
    )
